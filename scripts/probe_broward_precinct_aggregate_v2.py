...
def parse_table(table):
    ...
    for tr in table.find_all('tr'):
        ...
        if nums and namecell:rows.append((candidate_name(namecell),nums[-1]))
    return rows

def contest_context(table):
    for s in table.find_all_previous(string=True,limit=180):
        t=clean(s)
        if 'Contest:' in t:return clean(t.split('Contest:',1)[1])
        if re.match(r'^[A-Z0-9]+\s+-\s+.+',t) and len(t)<180:return clean(t.split(' - ',1)[1])
    return ''
...
def main():
    ...
    groups={};duplicate_precincts=[];examples=[];candidate_tables=0
    for idx,table in enumerate(soup.find_all('table')):
        ...
        candidate_tables+=1;sig=tuple(sorted(norm(n) for n,_ in rows))
        ...
        if precinct and precinct in g['precincts']:duplicate_precincts.append({'signature':sig,'precinct':precinct,'contest':contest,'tableIndex':idx})
    ...
    for sig,g in groups.items():
        ref=refs.get(sig);totals=dict(g['totals'])
        report.append({'signature':list(sig),'candidateNames':g['candidateNames'],'tables':g['tables'],'precinctCount':len(g['precincts']),'topContexts':sorted(g['contexts'].items(),key=lambda x:-x[1])[:5],'summedTotals':totals,'dosReference':ref,'dosMatch':bool(ref and totals==ref['votes'])})
